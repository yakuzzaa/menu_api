...
from serializers.menu import GetFullMenuSerializer, GetMenuSerializer
...
dish_services = DishServices(background_tasks=BackgroundTasks())
...
async def test_get_dish_empty_list(async_client: AsyncClient):
    ...
    dish = [GetDishSerializer.model_validate(item) for item in
            await dish_services.get_dish(create_menu['id'], create_submenu['id'])]
    json = response.json()
...
async def test_add_dish(async_client: AsyncClient):
    ...
    dish: GetDishSerializer = GetDishSerializer.model_validate(
        await dish_services.get_dish_by_id(create_menu['id'], create_submenu['id'], create_dish1['id']))
    obj_from_response = GetDishSerializer(**json)
    assert obj_from_response == dish


async def test_get_all_data(async_client: AsyncClient):
    response = await async_client.get(reverse_url('get_all'))
