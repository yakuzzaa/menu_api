from sqlalchemy import ResultProxy, and_, exists, select
...
class DishRepository(BaseRepository):
    ...
    async def read_by_id(cls, submenu_id, dish_id) -> Dish:
        async with async_session_maker() as session:
            query: ResultProxy = await session.execute(select(cls.model).filter_by(submenu_id=submenu_id, id=dish_id))
